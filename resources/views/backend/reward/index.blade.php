...
@section('content')
                ...
                <div class="row ">
                    <div class="col-sm-6 d-flex align-items-center">
                        <ol class="breadcrumb m-0">
                            ...
                            <li class="breadcrumb-item active"><a href="javascript:void(0)"> Rank and Reward Bonus</a></li>
                        </ol>
                    </div>
                    <div class="col-sm-6 d-flex flex-row-reverse align-items-center">
                        <a type="button" href="{{ route('admin.rewards.create') }}" class="btn btn-primary btn-xs">Add New</a>
                    </div>
                </div>
                <!-- row -->
                                            ...
                                            <tbody>
                                                @foreach($items as $key =>$value)
                                                    <tr>
                                                        ...
                                                        <td>{{ $value->b_team }}</td>
                                                        <td>{{ $value->c_team }}</td>
                                                        <td>{{ $value->amount }} <i class="fas fa-times"></i> {{$value->days}} Days</td>
                                                        <td>
                                                            @if($value->status == 1)
                                                        ...
                                                        <td>
                                                            <div class="dropdown ms-auto text-right" style="cursor: pointer;">
                                                                ...
                                                                <div class="dropdown-menu dropdown-menu-right">
                                                                    <a class="dropdown-item" href="{{route('admin.rewards.edit',['reward' => $value->id])}}" title="Edit"><i class="fas fa-pencil-alt" style="color: blue;"></i> Edit</a>
                                                                    
                                                                    <a class="dropdown-item" data-bs-target="#statusChange"
                                                                        href="javascript:void(0);" data-bs-toggle="modal" title="Status"
                                                                        onclick="statusChange('{{ route('admin.reward.status', $value->id) }}','Status Change')">
                                                                    ...
                                                                    <a class="dropdown-item" data-bs-target="#deleteConfirm" href="javascript:void(0);" 
                                                                        ...
                                                                        onclick="deleteConfirm('{{ route('admin.rewards.destroy',['reward' => $value->id])}}')"><i class="fa fa-trash" style="color: red;"></i> Delete</a>
    ...
    <input type="hidden" class="actionUrl" value="{{route('admin.reward.active.action')}}">
@endsection
@section('scripts')
    @parent
